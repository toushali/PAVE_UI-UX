# Kivie — Provider Portal (Pro2)

Static, clickable prototype of the Kivie Provider Portal, **Pro2 design** — a
fully separate look from Pro1. Same pages, layout, and content; different skin:

- **Dark-first**, with a **light/dark toggle** in the top bar (persisted; a tiny
  head bootstrap applies the theme before paint — also accepts `?theme=light|dark`).
- **Mono-forward technical typography** (IBM Plex Mono for every number, metric,
  timestamp, and the letterspaced uppercase micro-labels), teal/cyan accent.
- KPI cards carry a ring indicator + an "active" accent bar (per the reference).
- Kivie logo in the sidebar (light-ink wordmark on dark, dark-ink on light).

Theme tokens: [`css/tokens.css`](css/tokens.css) — `:root` = dark, `html[data-theme="light"]` = light.
