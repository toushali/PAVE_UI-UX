...
| 2 | **Guided MFA setup** — pick SMS/email OTP, large-text step-by-step (M1 §98, M3) | P-2 | 🟡 | `verify.html` enters an OTP, but there's **no first-time guided *setup* state** (choose channel → confirm). Add it |
| 3 | Welcome + contact confirm + **notification channel (SMS/email/both)** (M3 §194, §9.4) | P-3 | 🟡 | onboarding confirms email only — **add channel choice** (provider captures it at enrollment, but patient should confirm) |
...
| 10 | Settings / notifications: opt out of engagement reminders (M3 §193-197) | P-21 | 🟡 | reminder channel shown — **surface explicit SMS/email opt-out toggles** |
| 11 | Account / help / offline | P-22 | ✅ | present |

**Patient gaps:** guided MFA-setup state (P-2), onboarding channel choice (P-3), settings opt-out toggles (P-21). **No missing screens.**
...
| 5 | Enrollment → **plan review & Approve Plan** before activation, then invite (M2 §160-165, M3 §135-140) | PR-10,PR-13 | 🟡 | "Approve Plan" exists inside the Generated-Plan tab, but there's **no dedicated post-enrollment plan-review screen/state** — add it |
...
| 8 | **5-tier status badges** (M2 §51-56) | PR-20 | 🟡 | all tiers present, but **On Track = blue/teal; FSD §15.3 + M2 specify a *white* badge** — align or confirm deviation |
...
| 14 | Settings → **Stripe payment method / plan change** (Stripe Elements card, upgrade/downgrade) (M5 §34-46, §13) | PR-29 | 🟡 | Org section shows Subscription + "View invoices" only — **add card/payment-method + plan-change UI** |
| 15 | **API Cost Dashboard** — spend total · per-service breakdown · token log · trend charts · budget alerts · CSV (M5 §119-137, §12) | PR-30 | ❌ | **Missing screen — build & add to nav** |

**Provider gaps:** ❌ API Cost Dashboard; 🟡 dedicated Plan-Review/Approval screen; 🟡 On-Track badge colour; 🟡 Stripe card/plan-change UI.
...
| 5 | Provider mgmt — invite · **deactivate · manage roles** | OA-5 | 🟡 | invite ✅; **add explicit deactivate + role-edit** (only "Manage" today) |
...
that live *outside* the 15 client-facing screens. **None exist in `v1`/`pro1` today.**

| Surface (M-ref) | What it is | Status |
|---|---|---|
| **API Cost Dashboard** (M5 §12) | Operator cost view — spend, tokens, trends, budget alerts, CSV | ❌ *(also listed as provider #15 — decide where it lives)* |
| **Complaint Handling & MDR** admin UI (M2 §76-80, M4 §143-150, §15A) | Intake form, status (Open/Investigating/Resolved/Escalated), MDR escalation flag, 6-yr retention | ⬛ not designed |
| **Prompt Version Registry / Model Traceability** (M5 §95-117, §10.5) | Append-only prompt-version lookup, model-version per plan, admin query view | ⬛ not designed |
| **Backend Reports** (M5 §139-169, §14) | Provider-activity · patient-activity · AI/token-usage · **HIPAA audit** (PHI access, failed-auth, exports, admin actions, sessions) — all CSV | ⬛ not designed |
| **Data export / deletion** admin function (M1 §57, §3.4) | Export or permanently delete any patient's PHI | ⬛ not designed |

> These are genuine MVP UI deliverables in M4–M5, but they belong to a **Platform-Admin
> portal** that was intentionally left out of the two demo prototypes. **Decision needed:**
> design a lightweight Platform-Admin surface for the demo, or keep it backend-only.
...
| **On Track = white badge** | §15.3, M2 §53 | 🟡 | build uses blue/teal — **align to white, or confirm deliberate deviation** |
...
## 7 · Prioritised punch-list (design work remaining)

**A · Demo-critical (client-facing screens)**
1. **Provider · API Cost Dashboard** (PR-30) — only fully-missing core-adjacent screen.
2. **Provider · Plan Review & Approval** screen/state after enrollment (PR-10, PR-13).
3. **Patient · Onboarding** — guided MFA setup + notification-channel choice (P-2, P-3).

**B · Element polish / spec alignment**
4. **Provider · On-Track badge = white** per §15.3 (PR-20) — or confirm deviation.
5. **Org Admin · deactivate + role** controls (OA-5).
6. **Provider · Settings** — Stripe card / plan-change UI (PR-29).
7. **Patient** — settings SMS/opt-out toggles (P-21).

**C · New surface — decision required**
8. **Platform-Admin portal** (⬛): Complaint/MDR, Prompt Version Registry, Backend Reports + HIPAA audit, Data export/deletion. Design a light demo surface, or keep backend-only.

> **Multiplication note:** items live in **v1/pro1**. **pro2** is a colour theme of pro1 (inherits
> automatically); **v2** is a separate patient design, so patient additions (onboarding MFA/channel,
> opt-out toggles) must be mirrored into v2. Group A ≈ 3 screens; Group B ≈ small edits.
